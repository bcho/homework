...
{
    pos_t next_a, next_b, prev_a, prev_b;
    next_a = a->next; next_b = b->next;
    prev_a = a->prev; prev_b = b->prev;
    ...
    if (next_a == b) {
        ...
        next_a->prev = b; prev_a->next = b;
        next_b->prev = a; prev_b->next = a;
        b->next = next_a; b->prev = prev_a;
        a->next = next_b; a->prev = prev_b;
    }
}
